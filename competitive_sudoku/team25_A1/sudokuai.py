    ...
    def minimax_alpha_beta(self, game_state: GameState, max_depth, current_depth, alpha, beta, real_diff_score):
        ...
        if (self.player_number - len(game_state.moves)) % 2 == 1:  # our turn
            ...
            for move in moves:  # find a move maximize the min
                ...
                # print("current_depth ", current_depth)
                if current_depth == 0:
                    if eval_value > current_max:  # propose a current best move in depth=0
                        # print("!!!!!!!!!!!!Now I get a best Move:" + str(move))
                        # print("!!!!!!!!!!!!My advantage will be:" + str(eval_value))
                        best_move = [move]
                        if max_depth == 1:
                            random_move = random.choice(best_move)
                            self.propose_move(random_move)
                    if eval_value == current_max:
